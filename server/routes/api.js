  ...
  try {
    ...
    if (cookie) {
      try {
        ...
        allTorrents = resTorrents.data || [];
      } catch (e) {
        console.error("Debug endpoint failed to fetch torrents", e.message);
      }
    }
    ...
    for (const item of items) {
      ...
      if (allTorrents.length > 0) {
        const candidates = allTorrents.filter(t => 
          ...
          (item.path && t.content_path && t.content_path.includes(item.path.split(/[/\\]/).pop()))
        );
